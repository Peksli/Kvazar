...
namespace Kvazar {
		...
		uint32_t					 queueFamilyIndex)
	{
		...
		vkCreateCommandPool(logicalDevice, &poolInfo, nullptr, &commandPool.GetRaw());
	}
		...
		uint32_t				commandBufferCount)
	{
		...
		vkAllocateCommandBuffers(logicalDevice, &allocInfo, &cmdBuffer.GetRaw());
	}
		...
		VkFence					fence)
	{
		...
		vkQueueSubmit2(dstQueue, submitCount, pSubmits, fence);
	}
